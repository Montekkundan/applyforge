...
Rust was installed via homebrew, which can sometimes conflict with rustup-managed toolchains, especially for cross-compilation targets like wasm32-unknown-unknown. was getting errors with the `wasm32-unknown-unknown` target. I had to install it via `rustup target add wasm32-unknown-unknown`.